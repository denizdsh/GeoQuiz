...
export default function Stopwatch({ run, on, time, setTime, reversed = false, color = 'var(--text-primary)', width = 'fit-content' }) {
    useEffect(() => {
        let interval;
        if (run) {
    ...
    return (
        <article className={`stopwatch${reversed ? ' stopwatch-reversed' : ''}`} style={{ width }}>
