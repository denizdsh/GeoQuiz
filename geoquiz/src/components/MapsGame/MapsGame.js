...
const getRegion = (params, multiplayerData) => {
  return (
    params.subregion ||
    multiplayerData?.subregion ||
    params.region ||
    multiplayerData?.region
  );
};
...
function MapsGame({ title, multiplayerData = null }) {
  ...
  const region = getRegion(useParams(), multiplayerData);
    ...
    !ctx.country ? (
      ...
      <>
        ...
        <Map
          region={region}
          ctx={ctx}
          multiplayerUpdateScore={multiplayerUpdateScore}
        />
      </>
    )
  ) : (
    <GameStartMenu
      content={{ title, image: `/images/${region}/map.png` }}
...
function Map({ region, ctx, multiplayerUpdateScore }) {
  ...
  const handleClick = (country, position) => {
    if (!ctx.countries.concat(ctx.country).includes(country)) return;
    ...
    if (country === ctx.country) {
      ...
      if (isSmallCountry) {
        color = "purple";
        setSmallMarkers((smallMarkers) =>
          smallMarkers.filter((m) => m[0] !== country),
        );
      }
      setMarkers((markers) =>
        markers.concat({ country, position, isSmallCountry }),
      );
  ...
  const countryClickHandler = (e) => {
    ...
    if (color) e.sourceTarget.setStyle({ color });
  };
  ...
  const smallCountryIconUrl = `${
    process.env.NODE_ENV === "production"
      ? process.env.REACT_APP_URL
      : "http://localhost:3000"
  }/icons/star_icon_marker.png`;
  ...
  return (
    <MapContainer
      ...
      maxBounds={
        new LatLngBounds(
          coordinates[region].bounds.southWest,
          coordinates[region].bounds.northEast,
        )
      }
      placeholder={placeholder}
    >
      <TileLayer url={process.env.REACT_APP_MAPBOX_MAP_URL} />
      {geojson ? (
        <GeoJSON
          key="leaflet-map-geojson"
      ...
      {markers.map((m) => (
        <Marker
          key={`${m.country}-marker-${Math.random()}`}
          position={m.position}
          ...
          icon={
            new DivIcon({
              html: (() => {
                ...
                if (m.isSmallCountry) el.classList.add("purple-background");
      ...
      {smallMarkers?.map((m) => (
        ...
        <Marker
          key={`${m.country}-popup-${Math.random()}`}
          icon={
            new Icon({ iconUrl: smallCountryIconUrl, iconAnchor: [24, 48] })
          }
          country={m[0]}
          position={m[1]}
          eventHandlers={{
