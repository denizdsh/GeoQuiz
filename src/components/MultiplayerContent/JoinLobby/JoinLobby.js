...
export default function JoinLobby() {
  ...
  const { createPlayer, setLobby, player, resetLobby, lobby } =
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (newLobby.players && newLobby.players.hasOwnProperty(player.uid)) {
  ...
  useEffect(() => {
    // reset lobby in context after exiting page
    return () => resetLobby();
  }, [resetLobby]);
  ...
  return (
    <section className={styles.container + " slide"}>
      ...
      <Button className={styles.actionButton} onClick={joinHandler}>
        Join
      </Button>
    </section>
  );
